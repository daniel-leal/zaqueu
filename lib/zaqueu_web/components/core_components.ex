defmodule ZaqueuWeb.CoreComponents do
  @moduledoc """
  ...
  def input(assigns) do
    ~H"""
    <div phx-feedback-for={@name}>
      <.label for={@id} color={@label_color} label_class={@label_class}>
        <%= @label %>
      </.label>
      <input
        type={@type}
    ...
    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg my-4">
      <table class="min-w-full divide-y divide-gray-300 hover:table-fixed">
        ...
        <tfoot :if={@total_value != ""}>
          <tr>
            <td
              class="px-6 py-3 text-base bold font-semibold"
              colspan={@total_colspan}
            >
              Total
            </td>
            <td class={["px-3 py-5 text-sm font-semibold", @total_color]}>
              <%= @total_value %>
            </td>
          </tr>
        </tfoot>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ZaqueuWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ZaqueuWeb.Gettext, "errors", msg, opts)
    end
  end
